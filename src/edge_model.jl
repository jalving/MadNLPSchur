...
function EdgeModel()
    return EdgeModel(
        MadNLP.QPBlockData{Float64}(),
        nothing,
        MOI.NLPBlockData([], MadNLP._EmptyNLPEvaluator(), false),
        MOI.FEASIBILITY_SENSE
    )
end

# map each edge in graph to a model we can evaluate
function build_edge_model(
    edge::GOI.Edge;
    _differentiation_backend=MOI.Nonlinear.SparseReverseMode()
)
